...
void mkMTs(int newMTs)
{
 ...
 /* newgen -- is the number of MTs for which the coor
  ...
  /* Then, first randomely choose the Length of the newly generated MTs.
...
void getcoor(int istart, int n)
{
  ...
  int i,j,iMT,jMT,k,counter,iseg1,iseg2,r_count,a_count;
  float xcm,dist2,tmp,lb=0,rb=0;
  float nr,factor,r,criteria;
  int np;
  ...
  for (i=1;i<=n;i++)
      ...
      else
	{
	  ...
	  printf("N local MTs: %f\n",tmp);
        ...
        else if (polaritymodel==GC){
          float d_tip = (float) sampleExpDist(rb-lb);
          float A = 0.11 + 0.68*exp(-0.068*d_tip);
          float B = 0.40*exp(-0.03*d_tip);
            tmp = (A/(A+B));
    printf("New polarity ratio: %f\n",tmp);
            criteria=tmp;
        }
        
        else{
          printf("mkMT.c:> Erorr: polaritymodel not supported!\a\n");
          exit(0);
        }
